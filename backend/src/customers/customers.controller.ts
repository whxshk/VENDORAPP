import { Controller, Delete, Get, HttpCode, Param, Query, UseGuards, ForbiddenException, Post, Body } from '@nestjs/common';
...
export class CustomersController {
  ...
  async getMe(
    ...
    return this.customersService.getCustomerProfile(user.userId, tenantId);
  }

  @Get('me/memberships')
  @RequireScope('customer:*')
  @ApiOperation({ summary: 'Get current customer memberships' })
  @ApiResponse({ status: 200, description: 'List of loyalty memberships' })
  async getMyMemberships(
    @CurrentUser() user: { userId: string; tenantId: string },
    @TenantContext() tenantId: string,
  ) {
    return this.customersService.getCustomerMemberships(user.userId, tenantId);
  }

  @Delete('me')
  @HttpCode(204)
  @RequireScope('customer:*')
  @ApiOperation({ summary: 'Delete current customer account and all associated data' })
  @ApiResponse({ status: 204, description: 'Account deleted' })
  async deleteMe(
    @CurrentUser() user: { userId: string; tenantId: string },
    @TenantContext() tenantId: string,
  ): Promise<void> {
    return this.customersService.deleteCustomerAccount(user.userId, tenantId);
  }
  ...
  @Post(':id/adjust')
  @RequireScope('merchant:*')
  @ApiOperation({ summary: 'Manually adjust a customer\'s balance (fix accidents)' })
  ...
  async adjustBalance(
    ...
    return this.customersService.adjustBalance(tenantId, id, user.userId, Number(body.delta), body.reason || 'Manual adjustment');
  }
}
