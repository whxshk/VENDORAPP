...
import { OnboardingService, ConfigureLoyaltyDto } from './onboarding.service';
...
export class OnboardingController {
  ...
  @RequireScope('merchant:*')
  @ApiBearerAuth('JWT-auth')
  @ApiOperation({ summary: 'Configure loyalty program during onboarding' })
  @ApiResponse({ status: 201, description: 'Loyalty program configured' })
  async configureLoyalty(
    @Body() dto: ConfigureLoyaltyDto,
    @TenantContext() tenantId: string,
    @CurrentUser() user: any,
  ) {
    return this.onboardingService.configureLoyalty(tenantId, user.userId, dto);
  }
  ...
  @ApiResponse({ status: 201, description: 'Onboarding marked complete' })
  async completeOnboarding(
    @TenantContext() tenantId: string,
    @CurrentUser() user: any,
  ) {
    return this.onboardingService.completeMerchantOnboarding(tenantId, user.userId);
  }
}
