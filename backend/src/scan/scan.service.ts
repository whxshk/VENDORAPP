...
export class ScanService {
  ...
  async apply(
    ...
    if (!account) {
      // First scan with this merchant — auto-enroll the customer
      ...
      await account.save();
      await this.auditService.log(tenantId, staffUserId, 'CUSTOMER_ENROLLED', 'customer_merchant_account', account._id, {
        customerId: parsed.c,
      });
    }
    ...
    if (dto.purpose === 'PURCHASE') {
      ...
      // This takes precedence over the tenant's configured loyalty_type so that
      // merchants with stamps configured can still manually give points.
      const isStampMerchant = !!dto.stampRewardId;

      let result: { id: string; balance: number };
      let pointsIssued: number;
      const amount = typeof dto.amount === 'number' && dto.amount > 0 ? dto.amount : undefined;

      if (isStampMerchant) {
        // Stamp merchant: issue N stamps (amount field = stamp count, defaults to 1)
        pointsIssued = amount ? Math.max(1, Math.round(amount)) : 1;
        result = await this.transactionsService.issuePoints(
          tenantId,
          customerId,
          pointsIssued,
          dto.deviceId ?? null,
        ...
        await this.transactionModel.findByIdAndUpdate(result.id, {
          $set: {
            metadata: {
              ...
              stampRewardId: dto.stampRewardId ?? null,
      ...
      try {
        const ev = new this.scanEventModel({
          ...
          deviceId: dto.deviceId ?? undefined,
          purpose: 'PURCHASE',
          amount: amount ?? (isStampMerchant ? 1 : 0),
          status: 'OK',
          idempotencyKey,
        });
        await ev.save();
